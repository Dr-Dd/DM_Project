...
distinguish them (we could have also used a composite key of
\texttt{tconst} and \texttt{title}). This is a many-to-one
relationship to \texttt{titleBasics}. We also have the one-to-many
relationships with \texttt{region} and \texttt{language}. And finally,
two many-to-many relationships with \texttt{attributes} and with a new
entity called \texttt{akaType}.
...
\texttt{Gzip} library. The PostgreSQL library Psycopg imposes a buffer
of 64 KiB for its operations, which did not act as a bottleneck for
the transactions. We implemented the strategy pattern on the various
ways rows should be ingested by each different file and table, opening
one connection for each separate table, one file at a time, committing
the whole converted file at once. This approach, while fast enough,
could be problematic for databases that refuse multiple concurrent
connections from one host. A better way would be to generate the
non-transactional data to be loaded sequentially in the database.

\begin{figure}
  \begin{centering}
    \includesvg[width=1\textwidth,inkscapelatex=false]{imdb.sql.svg}
    \caption[IMDb Relational Schema]{IMDb's Relational Schema.}
...
processing of a single triple. This is a better approach for large
amounts of data.
...
by introducing SHACL, a validation protocol for N-Triples. This is
...
most common queries required by us and build what we need. But this is
costly and must be considered carefully, making graph databases
overall more versatile.

For example, comparing the same query for getting all the movies made
by one director, their titles, the director name and their year of
publishing, Apache Jena Fuseki spends 0.215 seconds, PostgreSQL 23.975
seconds. As such, graph databases allow for faster development and
expansion of functionalities, since implementing a new query for a
service may not need any external intervention from the engineer
other than writing the SPARQL code.

This ties well with another relatively minor problem: secondary
storage use. Our 225 million triple triplestore and its indexes
occupies a total of 19 GiB, our SQL database with its primary key
indexes occupies 22 GiB. This is another aspect which is better in the
world of graph databases (more utility in less space).
