...
In this document, we will qualitatively analyze the differences
between mapping a dataset to the relational model and to a graph
database model, specifically RDF. The dataset considered is IMDb's
Non-Commercial dataset, composed of information regarding movies,
actors, ratings, directors, writers and other attributes. As for
technology chosen, we used Apache Jena Fuseki for the Triplestore and
...
a schema if not via external documentation.
...
every primary key in every table we have. Secondary indexes need to be
built separately. In theory, we could map the most common queries
required by us and build what we need. But the construction of
multiple indexes can be costly and must be considered carefully,
making graph databases overall more versatile.
...
in the market. While SQL is inherently capable of addressing these
needs, it requires significantly more engineering. The aspects that
are better in SQL are exactly this granular quality and built-in
...
connected to the database. It is hard to think of a project in which
SQL really does not fit as a model. The worst that could happen is
that SQL is not automatically as performant natively, it needs lots of
engineering to be a good fit.
